...
ggplot(data, 
	aes(x= leafleng, y=leafwid,colour=who)) + 
	xlab("Leaf Length (mm)") +
	ylab("Leaf Width (mm)") +
	geom_density2d() + 
	theme(aspect.ratio = 1) +
...
cord <- data.frame(x_coord, y_coord, shape)
cord


ggplot(cord, aes(x=x_coord, y=y_coord, shape=shape)) + 
geom_point(size=30, colour="red") + 
scale_shape_manual(values=c(1,1,5)) +
